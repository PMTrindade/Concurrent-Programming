...
public class HashTable<K extends Comparable<K>, V> implements Map<K, V> {
	...
	public void singleReadLock(K key) {
		locks[calcTablePos(key)].readLock().lock();
	}

	@Override
	public void listReadLock(List<K> keys) {
		for (Integer i : sortedLockPositions(keys))
			locks[i].readLock().lock();
	}

	@Override
	public void singleReadUnlock(K key) {
		locks[calcTablePos(key)].readLock().unlock();
	}
	...
	public void singleWriteLock(K key) {
		locks[calcTablePos(key)].writeLock().lock();
	}
	...
	public void singleWriteUnlock(K key) {
		locks[calcTablePos(key)].writeLock().unlock();
	}
